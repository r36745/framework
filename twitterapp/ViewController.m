            ...
            [accountStored requestAccessToAccountsWithType:accountType options:nil completion:^(BOOL granted, NSError *error){
                if (granted)
                {
                    ...
                    if (twitterAccounts != nil)
                    {
                        ...
                        {
                            //userTimeLine = [NSString stringWithFormat:@"%@?%@&%@", @"https://api.twitter.com/1.1/statuses/user_timeline.json", @"screen_name=", @"count=10"];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    } else if ([segue.identifier isEqualToString:@"detailView"])
    {
        ...
        if (profileView != nil)
        {
            ...
            profileView.currentNumFriends = [lastPath valueForKeyPath:@"friends_count"];
